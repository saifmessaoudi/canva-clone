...
function dashboard({ doc }) {
    ...
    const [categorie , setCategorie] = useState([]);
      ...
      const deletedoc = async (data,index)=>{
    ...
    useEffect(()=> { 
      ...
      const handleSubmit = async (data) => {
        ...
        const ref = db.collection('Docs').doc("CV").collection('Doc')
        const ref1 = db.collection('Docs').doc("Business").collection('Doc')
        const ref2 = db.collection('Docs').doc("Lettre").collection('Doc')
        const ref3 = db.collection('Docs').doc("Papiertunisie").collection('Doc')
      ...
      const ModalComponent =  () => (
        ...
        >
          ...
          <div className=" flex justify-center flex-col w-full h-3/6  sm:w-3/6 sm:h-48  bg-gray-300 rounded-2xl opacity-100 outline-none px-4">
              ...
              <input ref={docNameFieldRef} type="text" placeholder="Add new document" className=" placeholder-gray-500 bg-transparent rounded-md p-2 mb-1 w-full outline-none" />
    ...
    <div class="flex flex-col flex-grow border-r border-gray-200 pt-5 pb-4 bg-white overflow-y-auto">
      <div class="mt-5 flex-grow flex flex-col">
        <div class="flex-1 px-2 bg-white space-y-1">
          ...
          <a href="categories" class="text-gray-600   hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
        ...
        <div class="flex-1 relative flex-shrink-0 overflow-y-auto focus:outline-none">
          <div class="py-6">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
              <h1 class="text-2xl font-semibold mb-7 text-gray-900">Add documents</h1>
              <p>Choose a categorie to add document</p>
...
<button  onClick={() => setShowModal(true)} className=' absolute flex right-2 hover:bg-gray-200 rounded-lg px-3'><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
